...
render(siteMainElement, new MoviesSectionView(), RenderPosition.BEFOREEND);
...
const closePopupElement = (moviePopupComponent) => {
  if (moviePopupComponent) {
    const onEscKeyDown = (evt) => {
      if (isEscEvent) {
        evt.preventDefault();
        siteFooterElement.removeChild(moviePopupComponent.element);
        document.removeEventListener('keydown', onEscKeyDown);
      }
    };
    document.addEventListener('keydown', onEscKeyDown);

    moviePopupComponent.setClickCloseHandler(() => {
      siteFooterElement.removeChild(moviePopupComponent.element);
      document.removeEventListener('keydown', onEscKeyDown);
    });
  }
...
const renderMovieCard = (movieContainer, movie) => {
  const movieCardComponent = new MovieCardView(movie);
  const moviePopupComponent = new PopupView(movie);

  render(movieContainer, movieCardComponent, RenderPosition.BEFOREEND);
  ...
  movieCardComponent.setClickHandler(() => {
    ...
    siteFooterElement.prepend(moviePopupComponent.element);
...
const renderExtraBlocks = () => {
  ...
  TITLES_FOR_EXTRA_BLOCKS.forEach((title) => {
    render(siteMoviesSectionElement, new MoviesExtraBlocksView(title), RenderPosition.BEFOREEND);
  });
